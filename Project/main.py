...
path_specific_emotion = "Data/HeartEmotions/sad.csv"
...
def recognition_emotion(train_data, path_test_emotion):
    X, y, groups, X_train, X_test, y_train, y_test = model.preparing_data(
        train_data)
    emotion = model.supervised_model(
        X, y, groups, X_test, y_test, path_test_emotion)
    return emotion


def App(path_train, path_dreamer, specific_emotion=False, **kwargs):
    path_specific_emotion = kwargs.get('path_specific_emotion', "None")
    path_ecg = kwargs.get('path_ecg', "None")
    heartRate = kwargs.get('heartRate', 80)
    try:
        exists(path_train)
    except:
        #print("Trainning Data not found, Will generate new data")
        if(exists(path_dreamer)):
            PE_dreamer(path_dreamer)
        # else:
            #print("DREAMER Dataset not found")
    finally:
        if(specific_emotion):
            path_ecg = path_specific_emotion
        ...
        return recognition_emotion(path_train, path_ecg)


print(App(path_train, path_dreamer, True,
      path_specific_emotion=path_specific_emotion))
